...
def calc_status(profit_cny, profit_rate):
    if profit_cny >= 200 and profit_rate >= 0.50: return "强烈推荐"
...
def main():
    ...
    for p in products:
        ...
        # 准入判断
        fails = []
        if profit_cny  < MIN_PROFIT_CNY:   fails.append(f"净利润 {profit_cny:.0f}元 < {MIN_PROFIT_CNY}元")
        if profit_rate < MIN_PROFIT_RATE:  fails.append(f"利润率 {profit_rate:.1%} < {MIN_PROFIT_RATE:.0%}")
        if supply_level == "稀缺":          fails.append("闲鱼供货稀缺")
        if fails:
            reason = "；".join(fails)
